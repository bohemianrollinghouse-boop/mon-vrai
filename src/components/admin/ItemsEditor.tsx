      ...
      <div className="grid grid-cols-3 gap-2 max-[749px]:grid-cols-2">
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-2 rounded-xl bg-paper px-3 py-2 text-[0.8125rem] font-semibold">
            ...
            <span className="flex gap-0.5 text-faint">
        ...
        {items.length < 12 && (
          <button type="button" onClick={() => setItems([...items, ""])} className="rounded-xl border-[1.5px] border-dashed border-line-warm px-3 py-2 text-[0.8125rem] font-semibold text-faint hover:border-ink hover:text-ink">
            + Ajouter
          </button>
        )}
      </div>
