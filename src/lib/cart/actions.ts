...
import { z } from "zod";
import { getCart, saveCart } from "@/lib/db/carts";
...
export async function addToCart(formData: FormData): Promise<CartActionResult> {
  ...
  await saveCart(id, { lines, promoCode: cart.promoCode });
  refresh();
...
export async function setPromoCode(formData: FormData): Promise<CartActionResult> {
  ...
  await saveCart(id, { lines: cart.lines, promoCode: parsed.data.code || undefined });
  refresh();
