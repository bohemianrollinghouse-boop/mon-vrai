...
export async function GET() {
  ...
  const [influencers, snap, campaigns] = await Promise.all([listInfluencers(), adminSnapshot(), listAllCampaigns()]);
  /* Le code et la remise d'aujourd'hui : ceux de la campagne en cours. */
  const live = (id: string) => liveCampaign(campaigns.filter((c) => c.influencerId === id).sort((a, b) => b.seq - a.seq));
  ...
  const lines = rows.map((r) => [r.influencer.name, mainAccount(r.influencer).handle, mainAccount(r.influencer).platform, live(r.influencer.id)?.code ?? "", live(r.influencer.id)?.discount ?? "", r.influencer.rate, `${process.env.NEXT_PUBLIC_SITE_URL ?? "https://monvrai.fr"}/?ref=${r.influencer.slug}`, r.influencer.active ? "Active" : "En pause", r.clicks, r.orders, r.byCode, r.byLink, euros(r.revenue), euros(r.commission)].map(cell).join(";"));
