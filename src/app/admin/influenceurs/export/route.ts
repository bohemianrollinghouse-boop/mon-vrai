...
export async function GET() {
  ...
  const lines = rows.map((r) => [r.influencer.name, r.influencer.handle, r.influencer.platform, r.influencer.code, r.influencer.discount, r.influencer.rate, `${process.env.NEXT_PUBLIC_SITE_URL ?? "https://monvrai.fr"}/?ref=${r.influencer.slug}`, r.influencer.active ? "Active" : "En pause", r.clicks, r.orders, r.byCode, r.byLink, euros(r.revenue), euros(r.commission)].map(cell).join(";"));
